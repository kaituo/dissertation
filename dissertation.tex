...
Dynamic analysis excels in precision and efficiency: it operates by executing programs with test cases, and subsequently is able to compute 100\% accurate results with relatively little cost; static analysis is good at generality: the results are able to generalize to future executions no matter what is the user input or what environment the system is in \cite{ernst03static}.
...
This chapter investigates the combination of static analysis and predictive and precise dynamic analysis.  We provide a general recipe for designing predictive and precise dynamic analysis for the first time. The key feature of our predictive and precise dynamic analysis is that it is much more precise (i.e., with fewer false warnings) than the static analysis alone, yet significantly more general (i.e., reporting more errors) than the dynamic tests in the programs test suite pertinent to the statically reported error. The idea of predictive and precise dynamic analysis has been implemented within FindBugs, a static bug detection tool.  We are able to improve both precision and recall of FindBugs-detected bugs.
